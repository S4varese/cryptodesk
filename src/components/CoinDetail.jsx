  ...
  return (
    <div className="flex items-center justify-between py-2.5 divider last:border-0">
      ...
      <div className="text-right">
        ...
        <span className="ml-2 text-xs font-mono" style={{ color: '#b0b8d0' }}>
...
export default function CoinDetail({ coin, pos, priceData, equityCurve, onBack }) {
  ...
  return (
    <div className="min-h-screen pb-24" style={{ background: '#f5f7ff' }}>
      ...
      <div className="sticky top-0 z-50 border-b px-4 py-3" style={{ background: '#ffffff', borderColor: '#eaedff' }}>
        <div className="max-w-lg mx-auto flex items-center gap-3">
          <button onClick={onBack} className="p-1.5 rounded-lg transition-colors"
            style={{ color: '#6b7280', background: '#f5f7ff' }}>
            <ArrowLeft size={16} />
          </button>
          <div className="flex items-center gap-2">
            <div className="w-7 h-7 rounded-lg flex items-center justify-center text-xs font-bold"
              style={{ background: 'rgba(26,86,255,0.1)', color: '#1a56ff' }}>
              {coin.slice(0, 2)}
            </div>
            <span className="text-sm font-bold tracking-wide" style={{ color: '#0a0a0a' }}>{coin} / EUR</span>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-lg mx-auto px-4 py-5 space-y-4">
        ...
        <div className="card-hero">
          ...
          <p className="text-4xl font-bold font-mono" style={{ color: '#0a0a0a' }}>€{fmt4(currentPrice)}</p>
          <div className={`flex items-center gap-1.5 mt-2 ${isUp ? 'text-emerald-500' : 'text-red-400'}`}>
            {isUp ? <TrendingUp size={13} /> : <TrendingDown size={13} />}
            <span className="text-sm font-semibold">
              {(priceData?.change || 0) >= 0 ? '+' : ''}{(priceData?.change || 0).toFixed(2)}% oggi
            </span>
            <span className="text-xs ml-1" style={{ color: '#b0b8d0' }}>
          ...
          <div className="card">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <p className="text-lg font-bold font-mono" style={{ color: '#0a0a0a' }}>{pos.qty.toFixed(5)}</p>
              </div>
              <div>
                <p className="label mb-1">Valore</p>
                <p className="text-lg font-bold" style={{ color: '#0a0a0a' }}>€{fmt2(currentValue)}</p>
              </div>
              {entryPrice && (
                <>
                  <div>
                    <p className="label mb-1">Prezzo entrata</p>
                    <p className="text-lg font-bold font-mono" style={{ color: '#6b7280' }}>€{fmt4(entryPrice)}</p>
                  </div>
                  <div>
                    <p className="label mb-1">P&L</p>
                    <p className={`text-lg font-bold ${isUp ? 'text-emerald-500' : 'text-red-400'}`}>
        ...
        {pos && (
          <div className="space-y-2 pt-1">
            {sellState === 'done' && (
              <div className="flex items-center justify-center gap-2 py-3.5 rounded-2xl text-sm font-semibold text-emerald-500"
            ...
            {sellState === 'confirm' && (
              <div className="rounded-2xl p-4 space-y-3"
                ...
                <p className="text-sm text-center" style={{ color: '#6b7280' }}>
                  ...
                  <span className="font-bold" style={{ color: '#0a0a0a' }}>~€{fmt2(currentValue)}</span>
                </p>
                <div className="grid grid-cols-2 gap-2">
                  <button onClick={() => setSellState('idle')}
                    className="py-2.5 rounded-xl text-sm font-semibold"
                    style={{ background: '#f5f7ff', color: '#6b7280', border: '1px solid #eaedff' }}>
                    Annulla
                  </button>
